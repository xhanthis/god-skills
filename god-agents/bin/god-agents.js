...
/** Agents generated by earlier versions and folded into others in 2.0. Removed on install when the file is recognisably ours. */
const RETIRED_AGENTS = ["god-architect", "god-cos", "god-dev", "god-police", "god-reverse", "god-scout", "god-security", "god-tester", "god-writer", "god-zen"];
const RETIRED_HOOKS = ["require-tester-pass.sh"];

/** True when the agent file exists and is one this tool generated for that name. */
function isOurRetiredAgent(file, name) {
  if (!fs.existsSync(file)) return false;
  const head = fs.readFileSync(file, "utf8").slice(0, 600);
  return new RegExp(`^name:\\s*${name}\\s*$`, "m").test(head) && /Subagent form of the/.test(head);
}

/** Removes retired agent files under <agentsDir>. Returns the names removed. */
function removeRetiredAgents(agentsDir) {
  const removed = [];
  for (const name of RETIRED_AGENTS) {
    const file = path.join(agentsDir, `${name}.md`);
    if (isOurRetiredAgent(file, name)) {
...
function printHelp() {
  const lines = [
    ...
    "  npx god-agents god-build god-qa      install only these agents",
...
async function installAgents(options) {
  ...
  const retired = removeRetiredAgents(agentsDir);
  ...
  if (retired.length > 0) {
    console.log(`${paint("–", "yellow")} retired agent(s) removed: ${retired.join(", ")}`);
  }
  console.log("");
...
function installHooks(options) {
  ...
  for (const old of RETIRED_HOOKS) {
    fs.rmSync(path.join(hooksDest, old), { force: true });
  }
...
function doctor() {
  ...
  for (const name of RETIRED_AGENTS) {
    if (isOurRetiredAgent(path.join(agentsDir, `${name}.md`), name)) {
      check(false, `retired agent ${name} still installed`, "run: npx god-agents (removes it)");
    }
  }
  for (const old of RETIRED_HOOKS) {
    if (fs.existsSync(path.join(GLOBAL_BASE, "hooks", "god", old))) {
      check(false, `retired hook ${old} still installed`, "run: npx god-agents --hooks (removes it)");
    }
  }
  check(
