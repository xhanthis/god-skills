...
| Skill | Owns |
|---|---|
| **god-build** | design + code, modes small / normal / deep, architecture pass, remove-first |
...
- **Bug or feature in code:** god-ceo (problem) → god-build → god-qa. Money in the diff → god-cfo after god-qa.
...
- **Rebuild an existing product:** god-pm (reverse pass) → god-build deep → god-qa.
- **Process / SOP:** god-pm (ops pass) → god-build if tooling → god-qa.
...
```json
{"chain": ["god-build", "god-qa"], "mode": "normal", "reason": "<one line>"}
```

- `chain` lists skill names in execution order, minimum set only; `mode` is god-build's mode when god-build is in the chain.
- The caller executes the chain sequentially and inline; god-ceo does not.
- Rework loops: god-qa FAIL → god-build → god-qa, max 3 cycles. god-cfo disagreement → god-build → god-cfo re-check. Conflicts between skills come back to god-ceo.
