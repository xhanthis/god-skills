...
- **AuthN/AuthZ:** every endpoint verifies identity AND permission; object-level check (user A cannot fetch user B's booking by changing an id); admin routes not reachable by role downgrade.
