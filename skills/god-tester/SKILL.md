...
- PII (name, phone, email, address, DOB, Aadhaar/PAN/passport, card/UPI/bank id, OTP, booking contact):
...
- **Fixture hygiene.** The tests you write go through CI secret scanning and the reviewer's injection detector.
  ...
  - Fixture people are obviously synthetic (`guest+test@example.invalid`), and tests never log them.
