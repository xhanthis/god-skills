...
6. **Unit economics before enthusiasm:** contribution margin per booking or order, CAC vs LTV, take rate, payback.
7. **Pricing** → `references/pricing.md`. **SQL, metrics, experiments** → `references/sql-metrics.md`.
8. **In code review:** every query obeys the reviewer's SQL rules (indexed `WHERE`, `LIMIT`, named columns, parameterized, no loops, batched writes; `BOOKING_ID` never matched by its numeric suffix); money is integers in the smallest unit or a decimal type, never floats; rounding happens once, at the boundary the definition names.
