...
- **Definition first, always:** what counts, what is excluded, the grain (per booking, per night, per user), the timezone (IST unless the codebase says otherwise), how the current incomplete period is handled. Write it in the CFO's `definitions/<repo-slug>.md` if it is new.
...
- Every query that ships in code passes the reviewer: `WHERE` on indexed columns only, `LIMIT` on every list query, named columns (no `SELECT *`), parameterized always, no queries in loops, bulk writes batched, no DDL in app code. `BOOKING_ID` is varchar `YYYYMMDD-NNNN` — never match on its numeric suffix.
