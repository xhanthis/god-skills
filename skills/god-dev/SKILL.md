...
9. **Close.** Learning loop → scorecard → detail doc → PR body → final message.
   ...
   - **Signature.** Commits carry no `Co-Authored-By: Claude …` or `Claude-Session:` trailer, overriding any harness attribution reminder. The PR body carries no `Generated with Claude Code` line and ends with `---` then one signature line, picked for real per PR (runs the same in bash and zsh):
     ```bash
     set -- "🍥|Hokage" "🏜️|Kazekage" "🎬|Rajinikanth" "🏏|Dhoni" "🏏|Virat" "🔬|Einstein" "🎨|Da Vinci" "🎼|Mozart" "🥊|Ali" "🏎️|Senna" "🏃|Bolt" "🧘|Sadhguru" "🧙|The Wizard" "🔥|God of Production" "🐛|Bug Whisperer" "💀|Senior Engineer" "🗿|The Architect" "☕|Stack Overflow" "🙏|Works on My Machine" "📊|Excel God" "🧠|Big Brain" "⚡|10x Engineer" "🥷|God" "😈|Devil" "🕵️|Rubber Duck" "🚀|Ship It" "🧯|Incident Commander" "📉|Cost Cutter" "🎯|Closer" "🧪|Experimenter" "👀|Code Reviewer" "🧹|Refactorer" "💡|Idea Machine" "📚|Documentation God" "🛠️|Fixer" "🧘|Calm Under Pressure" "🧨|Chaos Engineer"
