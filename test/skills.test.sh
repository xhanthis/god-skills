...
assert_contains "$TESTER" "deploy token when the user's local rules name one" "god-qa's PR-shape check reads the deploy token from local rules"
...
assert_contains "$DEV" "deploy token if the user's local rules" "god-dev takes the PR title's deploy token from local rules"
assert_eq "$(grep -rlE -- '--deploy|nodebackend|node backend' skills | wc -l | tr -d ' ')" "0" "no skill hardcodes a repo's deploy token"
